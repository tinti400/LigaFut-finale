...
for jogador in jogadores_filtrados:
    ...
    with col1:
        ...
        destino = jogador.get("destino", "nao_atribuido")
        if destino == "leilao":
            st.markdown("🟡 **Leilão**")
        elif destino == "mercado":
            st.markdown("🔵 **Mercado**")
        elif destino in mapa_times:
            st.markdown(f"🔴 **Atribuído ao {destino}**")
        else:
    ...
    with col_a2:
        if st.button("🛒 Mercado", key=f"mercado_{jogador['id']}"):
            dados_mercado = {
                ...
                "clube_original": jogador.get("clube_original", None)
            }
            try:
    ...
    with col_a3:
        if st.button("📢 Leilão", key=f"leilao_{jogador['id']}"):
            try:
                ...
                st.error(f"Erro ao enviar jogador ao leilão: {e}")
