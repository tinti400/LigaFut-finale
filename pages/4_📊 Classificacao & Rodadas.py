...
def calcular_renda_jogo(estadio, fase_time, rodada_atual, ultima_rodada):
    preco = float(estadio.get("preco_ingresso", 20.0))
    nivel = estadio.get("nivel", 1)
    capacidade = estadio.get("capacidade", 10000)

    fator_fase = 1 + (0.05 * fase_time)
    fator_ultima = 1.15 if rodada_atual == ultima_rodada else 1

    demanda_base = capacidade * (0.9 + nivel * 0.02) * fator_fase * fator_ultima
...
# 🧠 Calcular fase dos times
def calcular_fases(rodadas):
    ultimos = {}
    for rodada in reversed(rodadas):
        for jogo in rodada.get("jogos", []):
            for tipo, gols, adversario_gols in [("mandante", jogo.get("gols_mandante"), jogo.get("gols_visitante")), ("visitante", jogo.get("gols_visitante"), jogo.get("gols_mandante"))]:
                tid = jogo.get(tipo)
                if None in [tid, gols, adversario_gols]:
                    continue
                try:
                    gols, adversario_gols = int(gols), int(adversario_gols)
                except:
                    continue
                if tid not in ultimos:
                    ultimos[tid] = []
                if gols > adversario_gols:
                    ultimos[tid].append(1)
                elif gols < adversario_gols:
                    ultimos[tid].append(-1)
                else:
                    ultimos[tid].append(0)
    fases = {}
    for tid, resultados in ultimos.items():
        fase = sum(resultados[:3])
        fases[tid] = fase
    return fases
...
fases = calcular_fases(rodadas)
ultima_rodada = max([r["numero"] for r in rodadas]) if rodadas else 0
        ...
        if gm != "" and gv != "":
            try:
                ...
                if not check.data:
                    ...
                    if estadio:
                        fase = fases.get(m_id, 0)
                        renda, publico = calcular_renda_jogo(estadio, fase, rodada["numero"], ultima_rodada)
