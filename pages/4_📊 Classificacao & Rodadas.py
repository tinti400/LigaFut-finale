...
# 🔹 Seleção da divisão
divisao = st.selectbox("Selecione a divisão", ["Divisão 1", "Divisão 2", "Divisão 3"])
numero_divisao = divisao.split()[-1]

abas = st.tabs(["📅 Temporada 1", "📅 Temporada 2", "📅 Temporada 3"])

for i, aba in enumerate(abas, 1):
    with aba:
        numero_temporada = str(i)
        ...
        if rodadas_disponiveis:
            rodada_selecionada = st.selectbox("Escolha a rodada", rodadas_disponiveis, key=f"rodada_{numero_temporada}")
            for rodada in rodadas:
        ...
        if eh_admin:
            st.markdown("---")
            if st.button("🧹 Resetar Rodadas", key=f"reset_{numero_temporada}"):
                try:
