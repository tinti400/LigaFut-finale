...
if ativo and fase == "acao":
    ...
    try:
        ...
        if id_time == ordem[vez]:
            st.subheader("🔍 Escolha os jogadores para comprar com multa")
                ...
                with st.expander(f"📂 {time['nome']}"):
                    for jogador in elenco:
                        ...
                        if bloqueado:
                            ...
                            if not limite_alcancado and st.button(f"Multar {nome_j} (R$ {valor:,.0f})", key=btn_id):
                                saldo_r = supabase.table("times").select("saldo").eq("id", id_time).execute().data[0]["saldo"]
                                if saldo_r < valor:
                                    st.error("❌ Seu time não tem saldo suficiente para essa compra.")
                                    st.stop()
