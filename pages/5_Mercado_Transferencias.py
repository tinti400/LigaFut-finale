...
# 🧾 Elenco atual
res_elenco = supabase.table("elenco").select("nome", "posicao").eq("id_time", id_time).execute()
elenco_atual = res_elenco.data if res_elenco.data else []
qtde_elenco = len(elenco_atual)

# 👑 Admin seleciona jogadores
selecionados = set()
...
for jogador in jogadores_pagina:
    ...
    with col4:
        ...
        if is_admin:
            valor_padrao = int(jogador.get("valor", 1000000))
            novo_valor = st.number_input(
                f"Editar valor de {jogador['nome']}",
                min_value=1,
                step=1,
                value=valor_padrao,
                key=f"val_{jogador['id']}"
            )
        ...
        if qtde_elenco >= 35:
            ...
            if st.button(f"Comprar {jogador['nome']}", key=jogador["id"]):
                try:
                    ...
                    registrar_bid(id_time, "compra", "mercado", jogador["nome"], valor, jogador.get("origem"), nome_time)

                    st.success(f"✅ {jogador['nome']} comprado!")
                    st.experimental_rerun()

                except Exception as e:
                    st.error(f"Erro: {e}")

# 🚮 Admin exclui jogadores selecionados
