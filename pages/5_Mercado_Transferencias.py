...
total_paginas = (len(jogadores_filtrados) - 1) // jogadores_por_pagina + 1
pagina_atual = st.session_state.get("pagina_mercado", 1)
...
for jogador in jogadores_pagina:
    ...
    with col3:
        ...
        st.markdown(f"🏟️ Origem: {jogador.get('time_origem', 'Desconhecido')}")
...
with col1:
    if st.button("⬅ Página anterior") and pagina_atual > 1:
        st.session_state["pagina_mercado"] = pagina_atual - 1
        st.experimental_rerun()
with col3:
